...
The app is configured for deployment to Cloudflare Pages via Wrangler:
- **Configuration**: [wrangler.jsonc](APP90/wrangler.jsonc) defines the Pages project name and build output directory
- **Build output**: Vite builds to `dist/` directory
- **Deploy command**: `npx wrangler deploy` (requires Cloudflare account setup)
